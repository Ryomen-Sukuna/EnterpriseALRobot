...
def about(query: str, update: Update, context: CallbackContext) -> None:
    ...
    sql.update_user(user.id, user.username)
    about_text = f'\x1f    Kigyo (@{context.bot.username})\x1f    Maintained by [Dank-del](t.me/dank_as_fuck)\x1f    Built with ❤️ using python-telegram-bot v{__version__}\x1f    Running on Python {python_version()}\x1f    '
